...
const evidencePath = process.env.EVIDENCE_PATH || 'docs/prd/evidence/phaseb-controlled-rollout-round11.md';
...
function runPhaseBRound11() {
  ...
  // Controlled change #1: 2-step capped 800ms retry profile
  const retryAllowed1 = handlers.planRetry({
    ...
    payload: { probe: 'phaseb-round11-retry-allowed-1' }
  });
  ...
  const retryAllowed2 = handlers.planRetry({
    ...
    attempt: 2,
    errorCode: 'upstream-timeout',
    payload: { probe: 'phaseb-round11-retry-allowed-2' }
  });
  ...
  const retryBlockedAttempt = handlers.planRetry({
    ...
    attempt: 3,
    errorCode: 'upstream-timeout',
    payload: { probe: 'phaseb-round11-retry-blocked-attempt' }
  });
  ...
  const retryBlockedNonRetryable = handlers.planRetry({
    ...
    payload: { probe: 'phaseb-round11-retry-blocked-non-retryable' }
  });

  assert(retryAllowed1.ok && retryAllowed1.scheduledInMs === 400, 'retry attempt 1 should schedule 400ms');
  assert(retryAllowed2.ok && retryAllowed2.scheduledInMs === 800, 'retry attempt 2 should schedule 800ms');
  ...
  // Controlled change #2: inbound relay rollout remains lockcheck+bob via sender policy + flags
  const ackLockcheck = handlers.ack({
    contextId: canonicalContextId,
    senderId: 'lockcheck',
    botId: 'relayA',
    source: 'bot-relay',
    payload: { text: 'round11-relay-allowed-lockcheck' }
  });
  ...
  const ackBob = handlers.ack({
    ...
    botId: 'relayA',
    source: 'bot-relay',
    payload: { text: 'round11-relay-allowed-bob' }
  });
  ...
  try {
    handlers.ack({
      ...
      botId: 'relayA',
      source: 'bot-relay',
      payload: { text: 'round11-relay-blocked-alice-pair-flag' }
    });
  } catch (err) {
  ...
  try {
    handlers.ack({
      ...
      botId: 'relayA',
      source: 'bot-relay',
      payload: { text: 'round11-wrong-context' }
    });
  } catch (err) {
  ...
  assert(aliceBlockedReason.includes('sender-policy-violation'), 'alice should be blocked by sender policy even if pair flag is true');
  ...
  return {
    kpi1: {
      name: 'limited retry/backoff automation (2-step capped 800ms)',
      retryAttempts,
      retrySuccess,
    ...
    kpi2: {
      name: 'inbound relay rollout by sender policy + pair flags (lockcheck+bob hard-gate)',
